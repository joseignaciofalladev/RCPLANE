...
void loop() {
  ...
  // FAILSAFE
  if (millis() - lastSignalTime > FAILSAFE_TIME) {setFailsafe();}
}

// FUNCTIONS 
int readPWM(byte pin) {return pulseIn(pin, HIGH, 25000);}
bool validPWM(int pwm) {return pwm >= PWM_MIN && pwm <= PWM_MAX;}
